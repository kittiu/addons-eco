...
class cash_register_vatinfo(osv.osv_memory):

    _name = "cash.register.vatinfo"
    _description = "Cash Register VAT Info"
    ...
    def _get_currency_id(self, cr, uid, context=None):
        currency = self.pool.get('account.bank.statement.line').browse(cr, uid, context['active_id']).journal_id.currency.id or False
        if not currency:
            currency = self._get_default_currency(cr, uid, context=context)
        return currency

    def _get_vatinfo_date(self, cr, uid, context=None):
        return self.pool.get('account.bank.statement.line').browse(cr, uid, context['active_id']).vatinfo_date or False

    def _get_vatinfo_number(self, cr, uid, context=None):
        return self.pool.get('account.bank.statement.line').browse(cr, uid, context['active_id']).vatinfo_number or False

    def _get_vatinfo_supplier_name(self, cr, uid, context=None):
        return self.pool.get('account.bank.statement.line').browse(cr, uid, context['active_id']).vatinfo_supplier_name or False

    def _get_vatinfo_tin(self, cr, uid, context=None):
        return self.pool.get('account.bank.statement.line').browse(cr, uid, context['active_id']).vatinfo_tin or False

    def _get_vatinfo_branch(self, cr, uid, context=None):
        return self.pool.get('account.bank.statement.line').browse(cr, uid, context['active_id']).vatinfo_branch or False

    def _get_vatinfo_base_amount(self, cr, uid, context=None):
        return self.pool.get('account.bank.statement.line').browse(cr, uid, context['active_id']).vatinfo_base_amount or False

    def _get_vatinfo_tax_id(self, cr, uid, context=None):
        return self.pool.get('account.bank.statement.line').browse(cr, uid, context['active_id']).vatinfo_tax_id.id or False

    def _get_vatinfo_tax_amount(self, cr, uid, context=None):
        return self.pool.get('account.bank.statement.line').browse(cr, uid, context['active_id']).vatinfo_tax_amount or False
    ...
    def do_add_vatinfo(self, cr, uid, ids, context=None):
        if context is None:
            context = {}
        cash_register_line_obj = self.pool.get('account.bank.statement.line')
        line_ids = context['active_ids']
        for data in self.browse(cr, uid, ids):
            cash_register_line_obj.action_add_vatinfo(cr, uid, line_ids, data,
                             context=context)
