...
class invoice_expense_expense(osv.osv):
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        default.update({'voucher_id': False, 'date_confirm': False, 'date_valid': False, 'user_valid': False, 'account_move_id': False})
        return super(invoice_expense_expense, self).copy(cr, uid, id, default, context=context)
    ...
    _name = "invoice.expense.expense"
    _inherit = ['mail.thread']
    _description = "Expense"
    _order = "id desc"
    _track = {
        'state': {
            'invoice_expense.mt_expense_approved': lambda self, cr, uid, obj, ctx=None: obj['state'] == 'accepted',
            'invoice_expense.mt_expense_refused': lambda self, cr, uid, obj, ctx=None: obj['state'] == 'cancelled',
            'invoice_expense.mt_expense_confirmed': lambda self, cr, uid, obj, ctx=None: obj['state'] == 'confirm',
        },
    }

    _columns = {
        'name': fields.char('Description', size=128, required=False, readonly=True, states={'draft':[('readonly',False)], 'confirm':[('readonly',False)]}),
        ...
        'partner_id': fields.many2one('res.partner', "Supplier", required=True, domain=[('supplier','=',True)], readonly=True, states={'draft':[('readonly',False)], 'confirm':[('readonly',False)]}),
        ...
        'line_ids': fields.one2many('invoice.expense.line', 'expense_id', 'Expense Lines', readonly=True, states={'draft':[('readonly',False)]} ),
    ...
    _defaults = {
        'company_id': lambda s, cr, uid, c: s.pool.get('res.company')._company_default_get(cr, uid, 'invoice.expense.expense', context=c),
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(invoice_expense_expense, self).unlink(cr, uid, ids, context)
    ...
    def expense_confirm(self, cr, uid, ids, context=None):
        for expense in self.browse(cr, uid, ids):
            if expense.user_id:
                self.message_subscribe_users(cr, uid, [expense.id], user_ids=[expense.user_id.id])
    ...
    def action_receipt_create(self, cr, uid, ids, context=None):
        ...
        for exp in self.browse(cr, uid, ids, context=context):
            if not exp.partner_id.property_account_payable.id:
                raise osv.except_osv(_('Error!'), _('The supplier must have a payable account set.'))
            ...
            acc = exp.partner_id.property_account_payable.id
            eml.append({
                    'type': 'dest',
                    'name': '/',
                    'price': total, 
            ...
            lines = map(lambda x:(0,0,self.line_get_convert(cr, uid, x, exp.partner_id, exp.date_confirm, context=context)), eml)
...
class invoice_expense_line(osv.osv):
    ...
    def _amount(self, cr, uid, ids, field_name, arg, context=None):
        if not ids:
            return {}
        cr.execute("SELECT l.id,COALESCE(SUM(l.unit_amount*l.unit_quantity),0) AS amount FROM invoice_expense_line l WHERE id IN %s GROUP BY l.id ",(tuple(ids),))
    ...
    _columns = {
        ...
        'expense_id': fields.many2one('invoice.expense.expense', 'Expense', ondelete='cascade', select=True),
...
class account_move_line(osv.osv):
    ...
    def reconcile(self, cr, uid, ids, type='auto', writeoff_acc_id=False, writeoff_period_id=False, writeoff_journal_id=False, context=None):
        ...
        expense_obj = self.pool.get('invoice.expense.expense')
